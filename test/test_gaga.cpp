...
int main(int argc, char** argv) {
	...
	model.register_fitness_cb([](gaga::individual& ind) {
		return 6 - (ind.get_gene<int>("x") + ind.get_gene<int>("y") + ind.get_gene<int>("z"));
	});

	model.set_population_size(16);
	model.set_mutation_rate(12);

	model.register_initialization_cb([&model](gaga::population& ppl) {
	...
	model.register_selection_cb([&model](gaga::population& ppl) {
		ppl.remove_individuals_backward(model.get_population_size() >> 1);
	});

	model.register_crossover_cb([&model](gaga::population& ppl) {
		const auto n_pairs = ppl.size() >> 1;
		for (size_t i = 0; i < n_pairs; i++) {
			gaga::individual& p_1 = ppl.get_individual((i << 1));
			gaga::individual& p_2 = ppl.get_individual((i << 1) + 1);
			gaga::individual c_1{model};
	...
	model.register_mutation_cb([&model](gaga::population& ppl) {
